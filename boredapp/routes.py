...
from boredapp.boredAppFunctions import (
    check_if_activity_is_in_favourites,
    check_if_strong_password,
    display_the_activity,
    get_user_firstname,
    get_user_id,
    is_user_logged_in,
    reset_user_password,
)
from boredapp.forms import ForgotPassword, LogInForm, ResetPassword, SignUpForm
from boredapp.models import Favourites, TheUsers

from . import app, connect_to_api, database
from .config import GOOGLE_CLIENT_ID, MYEMAIL, MYEMAILPASSWORD, client_secrets_file
...
def callback():
    ...
    user_exists = (
        ...
        .filter(
            TheUsers.Email == session["Email"], TheUsers.Token == session["user_id"]
        )
        .first()
    )

    # Make sure we dont have a user with the same email address but different token
    email_exists: TheUsers = (
        database.session.query(TheUsers)
        .filter(
            TheUsers.Email == session["Email"], TheUsers.Token != session["user_id"]
        )
    ...
    if email_exists is not None:
        return render_template(
            "error.html",
            error_message="There is already an account with this e-mail address. Please use a different e-mail address to sign up.",
        )
...
def sign_up():
    ...
    if request.method == "POST":
        if (
            ...
            and check_if_strong_password(form.password.data) is True
        ):
            ...
            if (
                ...
                .filter((TheUsers.Email == email) | (TheUsers.Username == username))
                .first()
            ):
...
def login():
    ...
    if request.method == "POST" and form.validate_on_submit():
        ...
        if "@" in email_or_username:
            session[
                "Email"
            ] = email_or_username  # saves the users email into the session
            user = (
                database.session.query(TheUsers)
                .filter_by(Email=email_or_username)
                .first()
            )
            googleAccountUser = (
                database.session.query(TheUsers)
                .filter_by(Email=email_or_username, Password=None)
                .first()
            )
            ...
            user = (
                database.session.query(TheUsers)
                .filter_by(Username=email_or_username)
                .first()
            )

        if googleAccountUser:
            flash(
                "A Google account already exists with this email. Please login with Google.",
                "error",
            )
            ...
            session[
                "UserID"
            ] = get_user_id()  # save the users ID to a session for later use
    ...
    return render_template(
        "login.html", email_or_username=email_or_username, password=password, form=form
    )
    ...
    if form.validate_on_submit():
        ...
        if (
            ...
            with smtplib.SMTP("smtp.gmail.com", 587) as smtp:
                ...
                smtp.sendmail(
                    "from@example.com", user_email, message
                )  # Email the user with a link to reset their password

        flash(
            "A password reset link will be sent to this email if this user exists.",
            "success",
        )
...
def reset_password(token):
    ...
    try:
        email = ts.loads(
            token, salt="reset-password", max_age=900
        )  # token is valid for 15 mins max
    except Exception:
        flash(
            "Expired token. Go back to forgot password to generate a new link.", "error"
        )
...
def view_favourites_by_activity_type():
    ...
    favourites_by_activity_type = (
        ...
        .filter_by(UserID=session["UserID"], type=activity_type)
        .all()
    )

    return render_template(
        "favourites.html", users_favourites=favourites_by_activity_type
    )
...
def delete_favourite(activity_id):
    ...
    return redirect(
        request.referrer
    )  # redirect the user back to the previous page they were on before clicking the "delete" button.
...
def random_activity():
    ...
    if request.method == "POST":
        ...
        return render_template("user.html", activityInfo=activity_info, clicked=clicked)
...
def participant_number():
    ...
    if request.method == "POST":
        ...
        if participant_no_valid := regex_requirements.fullmatch(number_of_participants):
...
def activity_type():
    ...
    if request.method == "POST":
        ...
        regex_requirements = re.compile(
            r"\b(education|recreational|social|diy|charity|cooking|relaxation|music|busywork)\b",
            re.IGNORECASE,
        )
...
def save_activity():
    ...
    return render_template(
        "user.html", activityInfo=activity_info, clicked=True, link_str=link_str
    )
