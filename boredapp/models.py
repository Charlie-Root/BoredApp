...
if __name__ == "__main__":
    from config import DATABASENAME, DATABASEPASSWORD, HOST, USER
else:
...
class TheUsers(Base):
    ...
    UserID = database.Column(
        database.Integer, primary_key=True, autoincrement=True, nullable=False
    )
    ...
    Email = database.Column(
        database.String(65), nullable=False, unique=True, index=True
    )  # create index for Email column
    Username = database.Column(
        database.String(200), index=True
    )  # create index for Username column
    Password = database.Column(database.String(200))
    Token = database.Column(database.String(200))

    def __init__(
        self, FirstName, LastName, Email, Token=None, Username=None, Password=None
    ):  # Set Username & Password to Default None becuase if a user signs up with google account then those arguments aren't needed
...
class Favourites(Base):
    ...
    UserID = database.Column(
        database.Integer, database.ForeignKey("the_users.UserID"), nullable=False
    )
...
engine = create_engine(
    "mysql+mysqlconnector://{user}:{password}@{host}/{DatabaseName}".format(
        user=USER, password=DATABASEPASSWORD, host=HOST, DatabaseName=DATABASENAME
    )
)
